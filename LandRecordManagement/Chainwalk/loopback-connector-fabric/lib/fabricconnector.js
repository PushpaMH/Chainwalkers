...
class HFCSDKConnector extends Connector {
  ...
  getMember(username, password, client, lbConnector, org) {
    var tlsOptions = {
      trustedRoots: [],
      verify: false
    };
    var caUrl = lbConnector.settings[org].ca.url;
    var keyStoreOpts = {
      path: path.join(lbConnector.settings.keyStoreFile, '/', org),
    };
      ...
      .then((user) => {
        return new Promise((resolve, reject) => {
          if (user && user.isEnrolled()) {
            ...
            return resolve(user, "admin is already enrolled");
          }
          ...
          var cop = new copService(caUrl, tlsOptions, lbConnector.settings[org].ca.name, cryptoSuite);
          return cop.enroll({
            enrollmentID: username,
            enrollmentSecret: password
          }).then((enrollment) => {
            return member.setEnrollment(enrollment.key, enrollment.certificate, lbConnector.settings[org].mspid);
          }).then(() => {
  ...
  enrollAdmin(lbConnector, org) {
    ...
    cryptoSuite.setCryptoKeyStore(Client.newCryptoKeyStore(keyStoreOpts));
    client.setCryptoSuite(cryptoSuite);
    return Client.newDefaultKeyValueStore(keyStoreOpts)
      .then((store) => {
        client.setStateStore(store);
        return self.getMember("admin", "adminpw", client, lbConnector, org);
      });
  }

  registerNewUser(username, password, enrollmentID, enrollmentSecret, lbConnector, org) {
    var tlsOptions = {
      trustedRoots: [],
      verify: false
    };
    ...
    client.setCryptoSuite(cryptoSuite);
    var caUrl = lbConnector.settings[org].ca.url;
    var CA_client_obj = new copService(caUrl, tlsOptions, lbConnector.settings[org].ca.name, cryptoSuite);
    return Client.newDefaultKeyValueStore(keyStoreOpts)
      .then((store) => {
        client.setStateStore(store);
        return self.getMember(username, password, client, lbConnector, org);
      }).then((admin) => {
        logger.info("Successfully enrolled for admin");
        var req = {
        ...
        return CA_client_obj.register(req, admin);
      }).then(() => {
        return CA_client_obj.enroll({
          enrollmentID: enrollmentID,
          enrollmentSecret: enrollmentSecret
        });
      }).then((enrollment_cert) => {
        logger.info('Successfully enrolled user \'' + enrollmentID + '\'');
        member = new User(enrollmentID, client);
        return member.setEnrollment(enrollment_cert.key, enrollment_cert.certificate, lbConnector.settings[org].mspid);
      }).then(() => {
        var skipPersistence = false;
        if (!client.getStateStore()) {
          skipPersistence = true;
  ...
  getUserSubmitter(client, enrollmentID, enrollmentSecret, lbConnector, org) {
    var tlsOptions = {
      trustedRoots: [],
      verify: false
    };
    ...
    return Client.newDefaultKeyValueStore(keyStoreOpts)
      .then((store) => {
        client.setStateStore(store);
        return client.getUserContext(enrollmentID, true)
          .then((user) => {
            return new Promise((resolve, reject) => {
              return resolve(user);
            });
  ...
  enrollUser(enrollmentID, enrollmentSecret, lbConnector, org) {
    var tlsOptions = {
      trustedRoots: [],
      verify: false
    };
          ...
          .then((user) => {
            return new Promise((resolve, reject) => {
              if (user && user.isEnrolled()) {
                ...
                return reject("user: " + enrollmentID + " is not enrolled");
              }
            });
          })
          .catch((err) => {
  ...
  getAdminSubmitter(client, lbConnector, msppath, org) {
    var keyPath = msppath + "/" + "keystore";
    var certPath =  msppath + "/" + "signcerts";
    // if (org=="org1"){
    //   keyPath = path.join(__dirname, '../../../artifacts/crypto-config/peerOrganizations/peerOrg1/users/Admin@peerOrg1/msp/keystore');
    //   certPath= path.join(__dirname, '../../../artifacts/crypto-config/peerOrganizations/peerOrg1/users/Admin@peerOrg1/msp/signcerts');
    // }else{
    //   keyPath = path.join(__dirname, '../../../artifacts/crypto-config/peerOrganizations/peerOrg2/users/Admin@peerOrg2/msp/keystore');
    //   certPath= path.join(__dirname, '../../../artifacts/crypto-config/peerOrganizations/peerOrg2/users/Admin@peerOrg2/msp/signcerts');
    // }
    var keyPEM = Buffer.from(Common.readAllFiles(keyPath)[0]).toString();
    var certPEM = Common.readAllFiles(certPath)[0];
    return Promise.resolve(client.createUser({
      username: 'peer' + org + 'Admin',
      mspid: lbConnector.settings[org].mspid,
      cryptoContent: {
   ...
   */
  postChaincodes(peers, chaincode, lbConnector, msppath, org) {
    process.env.GOPATH = path.join(__dirname, '../../../../go/');
    console.log("GOPATH . . . - " + path.join(__dirname, '../../../../go/'));
    ...
    var clientPromise = Common.getClient(lbConnector.settings, org);
    var peerArrayPromise;
    if (peers !== undefined) {
      peerArrayPromise = Common.getPeers(lbConnector.settings, org, peers);
    } else { //Get all known peers
      peerArrayPromise = Common.getPeers(lbConnector.settings, org);
    }
    ...
    return Promise.all([clientPromise, peerArrayPromise]).then(
      (data) => {
        theClient = data[0];
        peerArray = data[1];
        return this.getAdminSubmitter(theClient, lbConnector, msppath, org);
      }).then((admin) => {
   ...
   */
  //not used as of now
  getChaincodesId(id, peers, lbConnector, msppath, org) {
    ...
    return Promise.all([clientPromise, peerArrayPromise]).then((data) => {
      ...
      var promises = [];
      peerArray.forEach(function(aPeer, index) {
   ...
   *
   */
  postChannelsChannelName(channelName, channelRequest, lbConnector, msppath) {
    ...
    //1. Get client
    var clientPromise = Common.getClient(lbConnector.settings, "org1");
    //2. Get Orderer
    ...
    return Promise.all([clientPromise, ordererPromise]).then((data) => {
      ...
      return this.getAdminSubmitter(theClient, lbConnector,msppath, "org1");
    }).then((admin) => {
   ...
   *
   */
  putChannelsChannelName(channelName, channelRequest, lbConnector, msppath) {
   ...
   */
  postChannelsChannelNameChaincodes(channelName, peers, chaincode, lbConnector, msppath, org) {
    ...
    return Common.getClientWithChannels(lbConnector.settings, org).then((aClient) => {
      ...
      if (failed > 0) {
        ...
        return Promise.reject(resp.peerResponses[0][0]);
      }
   ...
   * @returns {Promise} Resolving to the result
   */
  putChannelsChannelNameChaincodes(channelName, peers, chaincode, lbConnector,msppath, org) {
   ...
   * @returns {Promise}
   */
  postChannelsChannelNameEndorse(channelName, peers, transaction, lbConnector, msppath, org) {
    ...
    if (transaction.proposal === undefined) {
      ...
      if (transaction.proposal.chaincodeId === undefined) {
        errMsg = "proposal.chaincodeId "
      }
    ...
    return Common.getClientWithChannels(lbConnector.settings, org).then((aClient) => {
      ...
      if (transaction.proposal.txId === undefined) {
        endorseRequest.txId = theClient.newTransactionID();
      }
   ...
   * @returns {Promise}
   */
  postChannelsChannelNameTransactions(channelName, transaction, lbConnector, enrollmentID, enrollmentSecret, isAdmin, msppath, org) {
    ...
    return Common.getClientWithChannels(lbConnector.settings, org).then((aClient) => {
      ...
      if (isAdmin == true) {
        ...
        return this.getUserSubmitter(theClient, enrollmentID, enrollmentSecret, lbConnector, org);
      }
    }).then((user) => {
      ...
      if (transaction.proposalResponses === undefined ||
        ...
        // Generate a transaction ID if needed.
        if (transaction.proposal.txId === undefined) {
          txId = theClient.newTransactionID();
       ...
       res_payload = (tranReq["proposalResponses"])[0].response.payload.toString('utf8');
      ...
      if (res_payload != "\"\""){
        response.bc_response = res_payload;
      }
      if (broadcastResponse.status === "SUCCESS") {
        return Promise.resolve(response);
        //return Promise.resolve(((pass_results[0])[0]).response.payload);
      } else {
   ...
   * @returns Promise
   */
  getChannels(lbConnector, org) {
    Common.logEntry(logger, this.getChannels);

    var clientPromise = Common.getClient(lbConnector.settings, org);
    var peerPromise = Common.getPeer(lbConnector.settings, org);
   ...
   * @param {object} lbConnector The loopback connector object
   */
  getChannelsChannelName(channelName, lbConnector,msppath, org) {
    ...
    return Common.getClientWithChannels(lbConnector.settings, org).then((aClient) => {
      ...
      theChannel = aClient.getChannel(channelName);
      return this.getAdminSubmitter(aClient, lbConnector, msppath, org);
    }).then((admin) => {
      return theChannel.initialize();
    }).then((ignored) => {
      response = theChannel;
   ...
   * @param {object} lbConnector The loopback connector object
   */
  postChannelsChannelNamePeers(channelName, peerInfo, lbConnector, msppath, org) {
    ...
    return Common.getClientWithChannels(lbConnector.settings, org).then((aClient) => {
      ...
      for (var i = 0; i < peerInfo.length; i++) {
        aPeer = new Peer(peerInfo[i].url, peerInfo[i].opts);
        targets.push(aPeer);
      }
      joinChannelRequest.targets = targets;
   ...
   */
  //not used..
  getChannelsChannelNameChaincodesId(channelName, id, lbConnector, msppath, org) {
   ...
   * @param {object} lbConnector The loopback connector object
   */
  getChannelsChannelNameChaincodes(channelName, lbConnector, msppath, org) {
   ...
   * @param {object} lbConnector The loopback connector object
   */
  postChannelsChannelNameLedger(channelName, chaincodeId, blockId, blockHash, txnId, body, lbConnector, enrollmentID, enrollmentSecret, isAdmin, msppath, org) {
    ...
    return Common.getClientWithChannels(lbConnector.settings, org).then((aClient) => {
      ...
      try {
        if (queryResult instanceof Array && queryResult[0] instanceof Buffer) {
          ...
          var res_str = response.toString('utf8');
          if (res_str.includes("Error: chaincode error")){
            return Promise.reject(response);
          }
        }
      } catch (err) {
   ...
   */
  //not being used
  getChannelsChannelNameBlocks(channelName, blockId, blockHash, lbConnector, msppath, org) {
  ...
  getChannelsChannelNameTransactionsTransactionID(channelName, transactionID, lbConnector, msppath, org) {
