...
//utility function for reading all the files under a directory
var readAllFiles = function(dir) {
...
*/
exports.getClientWithChannels = function(settings, org){
  //1. Get a new Client instance.
    return getClient(settings, org).then( (aClient)=>{
      ...
      return addChannelsToClient(aClient,settings,org);
    }).catch((err)=>{
      logger.error(err);
      return Promise.reject(err);
    });
...
*/
exports.getPeer = function(settings, org){
  if(settings[org].peers !== undefined && settings[org].peers !== null){
    //Get org from datasources.json
    //var orgIndex = settings[org].peers[0].orgIndex;
    let opts = {};
    // Set the PEM to be the org's CACertFile
    opts.pem = fs.readFileSync(settings[org].CACertFile,'utf-8');
    //console.log(opts.pem);
    //TODO Do the following better!
    if(settings[org].peers[0].hostname!== undefined && settings[org].peers[0].hostname !== null){
      //Set ssl-target-name-override to let tls work in test environment
      opts['ssl-target-name-override'] = settings[org].peers[0].hostname;
    }
    var peerUrl = settings[org].peers[0].requestURL;
    ...
    return Promise.reject("peers not configured in datasources.json");
  }
}
...
*/
var getPeers = function(settings,org,peersIndex){
  var subset = false;
  var resultLength = 0;
  ...
  if(settings[org].peers !== undefined && settings[org].peers !== null){
    if(resultLength == 0){
      resultLength = settings[org].peers.length;
    }
    var resultArray = new Array(resultLength);
    var outputIndex = 0;
    settings[org].peers.forEach( function(aPeer,index){
      ...
      // Set the PEM to be the org's CACertFile
      opts.pem = fs.readFileSync(settings[org].CACertFile,'utf-8');
      //var data = fs.readFileSync(path.join(__dirname, '../../fabric-rest/', aPeer.publicCertFile));
...
*/
var addChannelsToClient = function(aClient, settings, org){

  var orderersPromise = getOrderers(settings);
  var peersPromise = getPeers(settings, org);
  ...
  return Promise.all([peersPromise,orderersPromise]).then( (data)=>{
      ...
      settings.channels.forEach( function(channelConfig,cIndex){
          ...
          channelConfig.peersIndex[org].forEach(function(peerArrayIndex,pIndex){
...
exports.sendTxProposal = function(channel,endorseRequest){
  return channel.sendTransactionProposal(endorseRequest).then((ProposalResponseObject)=>{
    ...
    logger.debug("sendTxProposal() resolved.");
    var res_str = JSON.stringify(ProposalResponseObject[0][0]);
    if(res_str.includes("\"status\":200")){
      return Promise.resolve(tranRequest);
    }else{
      return Promise.reject(ProposalResponseObject[0][0]);
    }
  }).catch((err)=>{
    logger.debug("sendTxProposal() not resolved.");
    return Promise.reject(err);
  });
}
  ...
  if(response instanceof Buffer){
    ...
    if(respJSON.startsWith("{")) {
      ...
      return resp;
        ...
        logger.debug("starts with quotes . . .");
      }
    else if(respJSON === ("")){
